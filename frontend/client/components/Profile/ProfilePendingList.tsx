...
const STATE = {
  publishedId: null as null | UserProposal['id'],
};
...
class ProfilePendingList extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <>
        {proposals.map(p => (
          <ProfilePending key={p.id} proposal={p} onPublish={this.handlePublish} />
        ))}
  ...
  private handlePublish = (publishedId: UserProposal['id']) => {
