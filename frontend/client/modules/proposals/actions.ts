...
function addProposalUserRoles(p: Proposal, state: AppState) {
  if (state.auth.user) {
    ...
    if (p.team.find(t => t.id === authUserId)) {
      p.isTeamMember = true;
    }
  }
  return p;
...
export function fetchProposal(proposalId: Proposal['id']) {
  return async (dispatch: Dispatch<any>, getState: GetState) => {
    dispatch({
...
export function fetchProposalComments(id?: number) {
  return async (dispatch: Dispatch<any>, getState: GetState) => {
    ...
    const proposalId = id || state.proposal.detail.id;
    dispatch({
...
export function fetchProposalUpdates(proposalId: Proposal['id']) {
