export function getAmountError(amount: number, max: number = Infinity, min?: number) {
  if (amount < 0) {
    ...
    amount.toFixed(0).length < amount.toString().length ||
    amount.toString().includes('1e')
  ) {
    return 'Must be in increments of 1';
  } else if (amount > max) {
    return `Cannot exceed maximum (${max} STARS)`;
  } else if (min && amount < min) {
    return `Must be at least ${min} STARS`;
  }
