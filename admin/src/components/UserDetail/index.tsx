...
class UserDetailNaked extends React.Component<Props, State> {
  ...
  render() {
    ...
    if (!u || (u && u.id !== id) || userDetailFetching) {
      return 'loading user...';
    }
      ...
      <Popconfirm
        ...
        title={
          <>
            ...
            <strong>All</strong> data associated with this user will be deleted.
          </>
        }
        okText="Delete"
        cancelText="Cancel"
    ...
    return (
      <div className="UserDetail">
        ...
        <h1>
          <UserAvatar size="default" shape="square" icon="user" user={u} />
          <span>
              ...
              >
                <List
                  ...
                  renderItem={(p: Proposal) => (
                    <List.Item
                      actions={[
                        <Link key="view" to={`/proposals/${p.id}`}>
                          view
                        </Link>,
                      ]}
                    >
  ...
  private handleDelete = async () => {
    ...
    await store.deleteUser(store.userDetail.id);
  ...
  private handleToggleSilence = async () => {
    if (store.userDetail) {
      ...
      await store.editUser(ud.id, { silenced: newSilenced });
      if (store.userSaved) {
        message.success(
          <>
  ...
  private handleToggleAdmin = async () => {
    if (store.userDetail) {
      ...
      await store.editUser(ud.id, { isAdmin: newAdmin });
      if (store.userSaved) {
        message.success(
          <>
    ...
    if (store.userDetail) {
      ...
      if (newBanned) {
        FeedbackModal.open({
          ...
          okText: 'Ban',
          onOk: async reason => {
            await store.editUser(ud.id, { banned: newBanned, bannedReason: reason });
            informSuccess();
          },
        });
      } else {
        Modal.confirm({
          ...
          onOk: async () => {
            await store.editUser(ud.id, { banned: newBanned });
            informSuccess();
          },
        });
      }
