...
import { Popconfirm, List, Icon, message } from 'antd';
  ...
  render() {
    ...
    return (
      <List.Item className="ModerationItem" actions={actions}>
        <List.Item.Meta
          avatar={<UserAvatar icon="user" shape="square" size={48} user={p.author} />}
          title={
            <>
              <Link to={`/users/${p.author!.id}`}>{p.author!.displayName}</Link>{' '}
