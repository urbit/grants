...
class Parser(FlaskParser):
    ...
    def parse_querystring(self, req, name, field):
        result = super().parse_querystring(req, name, field)
        # for GET query string lists of the form ?item[]=one&item[]=two
        if isinstance(result, _Missing) and isinstance(field, fields.List):
            result = req.args.getlist(f'{name}[]')
        return result
...
paginated_fields = {
    ...
    "filters": fields.List(fields.Str(), required=False, missing=[]),
