...
class AzimuthPoint(db.Model):
    __tablename__ = "azimuth_point"

    point = db.Column(db.String(255), primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)

    def __init__(self, point: str, user_id):
        self.point = point
        self.user_id = user_id
...
class User(db.Model, UserMixin):
    ...
    email_recovery = db.relationship(
        EmailRecovery,
        uselist=False,
        back_populates="user",
        lazy=True,
    ...
    proposal_teams = db.relationship(
        "Proposal",
    ...
    def set_azimuth(self, point: str):
        # Remove existing azimuth point if it was already added
        existing = AzimuthPoint.query.get(point)
        if existing:
            db.session.delete(existing)
        azimuth = AzimuthPoint(point, self.id)
        db.session.add(azimuth)
        db.session.flush()
...
class SelfUserSchema(ma.Schema):
    ...
    azimuth = ma.Nested("AzimuthPointSchema")
...
class AzimuthPointSchema(ma.Schema):
    class Meta:
        model = AzimuthPoint
        fields = ("point",)

azimuth_point_schema = AzimuthPointSchema()
azimuth_point_schemas = AzimuthPointSchema(many=True)
