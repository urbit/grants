...
import grant.rfw.models as rfw_models
...
from grant.utils.azimuth import validate_azimuth_signature
from .models import (
    User,
    SocialMedia,
    Avatar,
...
@query({
    ...
    "withWork": fields.Bool(required=False, missing=None),
})
...
@body({
    ...
    "azimuth": fields.Str(required=True, allow_none=True)
})
def update_user(user_id, display_name, title, social_medias, avatar, azimuth):
    user = g.current_user
    ...
    # only allow deletions for azimuth points
    if user.azimuth and not azimuth:
        db.session.delete(user.azimuth)
...
@blueprint.route("/me/azimuth", methods=["PUT"])
@auth.requires_auth
@body({
    "data": fields.Str(required=True),
    "signature": fields.Str(required=True),
})
def set_user_azimuth_point(data, signature):
    # Data comes in as a string to avoid animalification
    try:
        data = json.loads(data)
    except:
        return {"message": "Invalid JSON data"}, 400

    if not validate_azimuth_signature(data, signature):
        return {"message": "Invalid signature"}, 400

    g.current_user.set_azimuth(data['message']['point'])
    db.session.commit()
    print(g.current_user.azimuth)
    return user_schema.dump(g.current_user)
